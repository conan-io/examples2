...
class ArmToolchainPackage(ConanFile):
    ...
    def get_toolchain(self, target_arch):
        if target_arch in ["armv6", "armv7", "armv7hf"]:
            return ("x86_64-arm-none-linux-gnueabihf", 
                    "df0f4927a67d1fd366ff81e40bd8c385a9324fbdde60437a512d106215f257b3")
        else:
            return ("x86_64-aarch64-none-linux-gnu", 
    ...
    def build(self):
        toolchain, sha = self.get_toolchain(self.settings_target.arch)
        get(self, f"https://developer.arm.com/-/media/Files/downloads/gnu/13.2.rel1/binrel/arm-gnu-toolchain-13.2.rel1-{toolchain}.tar.xz",
            sha256=sha, strip_root=True)            

    def package(self):
        toolchain, _ = self.get_toolchain(self.settings_target.arch)
